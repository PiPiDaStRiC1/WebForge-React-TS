...
export const UserProvider = ({children}: UserProviderProps) => {
    ...
    const isAuthenticated = authData.isLoggedIn && !!authData.userId;
    ...
    const deleteUser = useCallback(() => {
        ...
        if (!authData.userId) throw new Error('Нет авторизованного пользователя');

        // eslint-disable-next-line
        const {[authData.userId]: _, ...restUsers} = users;
    ...
    useEffect(() => {
        try {
            ...
            const parsedUser = parsedUsers[authData.userId];
            
            setUserData(parsedUser);
        } catch(error) {
            setError(error instanceof Error ? error : new Error('Неизвестная ошибка при загрузке данных пользователя'));
        } 
    }, [authData.userId]);
    ...
    useEffect(() => {
        if (userData) {
            try {
                const users: StoredUsers = initUsers();
                if (!users) throw new Error('Failed to load users from localStorage in save userData useEffect');
