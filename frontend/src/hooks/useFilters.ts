...
export const useFilters = () => {
    ...
    const setRange = (
        key: string,
        [minValue, maxValue]: [string, string],
    ...
    const getRange = (key: string, defaultValue: [string, string]): [string, string] => {
        const value = searchParams.get(key);
        if (value) {
            const [min = defaultValue[0], max = defaultValue[1]] = value.split("-");
            return [min, max];
        } else {
            return defaultValue;
        }
