        ...
        return data.allIds
            ...
            .filter(
                (user) =>
                    ...
                    user.pricePerHour >= priceLow &&
                    user.pricePerHour <= priceHigh &&
                    (skills.length === 0 || skills.some((skill) => user.skills.includes(skill))) &&
                    user.rating >= ratingLow &&
                    user.rating <= ratingHigh &&
                    user.experience >= experienceLow &&
                    user.experience <= experienceHigh &&
                    (status.length === 0 || status.includes(user.status)) &&
                    user.completedOrders >= completedOrdersLow &&
                    user.completedOrders <= completedOrdersHigh,
            );
    }, [
        search,
        data,
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            ...
                            <div
                                className={`overflow-hidden transition-all duration-500 ease-in-out ${
                                    loadSkills ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
                                }`}
                            >
                                ...
                                <>
                                    {sortedData.length === 0 ? (
                                        <div className="flex flex-col gap-2 items-center justify-center py-15">
                                            <h1 className="h-full flex text-xl items-center justify-center">
                                                Ни один исполнитель не найден
                                            </h1>
                                        </div>
                                    ) : (
