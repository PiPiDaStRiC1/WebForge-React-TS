...
async function main() {
    ...
    try {
        ...
        // Update sequence values for Postgres (to avoid duplicate ID errors on insertion)
        console.log("Updating PostgreSQL sequence values...");
        await prisma.$executeRawUnsafe(
            `SELECT setval('"User_id_seq"', COALESCE((SELECT MAX(id) FROM "User"), 1));`,
        );
        await prisma.$executeRawUnsafe(
            `SELECT setval('"Picture_pictureId_seq"', COALESCE((SELECT MAX("pictureId") FROM "Picture"), 1));`,
        );
        await prisma.$executeRawUnsafe(
        ...
        await prisma.$executeRawUnsafe(
            `SELECT setval('"FreelancerSkill_skillId_seq"', COALESCE((SELECT MAX("skillId") FROM "FreelancerSkill"), 1));`,
        );
        await prisma.$executeRawUnsafe(
            `SELECT setval('"OrderSkill_skillId_seq"', COALESCE((SELECT MAX("skillId") FROM "OrderSkill"), 1));`,
        );
        await prisma.$executeRawUnsafe(
            `SELECT setval('"Response_id_seq"', COALESCE((SELECT MAX(id) FROM "Response"), 1));`,
        );
        await prisma.$executeRawUnsafe(
            `SELECT setval('"Favorite_id_seq"', COALESCE((SELECT MAX(id) FROM "Favorite"), 1));`,
        );
        await prisma.$executeRawUnsafe(
            `SELECT setval('"FavoriteOrder_id_seq"', COALESCE((SELECT MAX(id) FROM "FavoriteOrder"), 1));`,
        );
